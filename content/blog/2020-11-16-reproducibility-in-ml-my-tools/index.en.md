...
I've faced the problem of managing my results properly during my first machine learning project. Like any naive person, instead of reading the user manual "How to manage any results like a _champ_," I brute-forced my way to it by creating `.txt` files of my results.

It seemed nice: all I had to do was to create a "meaningful" filename, such as `param_1_param_2_..._param_100.txt` plus a timestamp. Afterwards, I just had to write down all my results. I was so naive: at one point, I've got more than 100 files for that project only. Now, try finding which experiment was the best.
...
Managing my configurations was the problem I've faced the most during my work. Machine learning models have plenty of parameters, and their settings can greatly impact performance. You might want to try different architectures, optimizers, and do a grid search on parameters. The problem is that it is difficult to swap between settings and adding new one can be cumbersome. My first solutions were [Argparse](https://docs.python.org/3/howto/argparse.html) and [Configparser](https://docs.python.org/3/library/configparser.html). These solutions are really useful since they are easy to set up, and arguments are readable. But the more you add parameters, the more the code becomes complex, and it becomes difficult to navigate it. Also, it feels strange to always just copypaste code to create new arguments, somewhat contradictory to the [Rules of three](<https://en.wikipedia.org/wiki/Rule_of_three_(computer_programming)#:~:text=It%20states%20that%20two%20instances,and%20attributed%20to%20Don%20Roberts>) idea that "If you copy-paste some bloc of code three times, you might create a function." I also tried other solutions such as [Sacred](https://pypi.org/project/sacred/) with JSON configuration files, but the framework is not well designed in my opinion. In summary, something is missing in all of these solutions.
