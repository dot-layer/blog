...
Le *package* R `argparse` est en fait une *wrapper* à la librairie Python du même nom. D'autres *packages* R, comme [optparse](https://cran.r-project.org/web/packages/optparse/index.html), fonctionnent de manière similaire. L'objectif de cet article n'est pas de vanter un *package* plus qu'un autre, mais plutôt d'illustrer le concept général en utilisant comme exemple le *package* `argparse`. La création d'une interface de ligne de commande avec ce *package* est très simple, les étapes sont généralement :
