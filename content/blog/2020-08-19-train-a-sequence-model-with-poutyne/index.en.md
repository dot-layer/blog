...
But, training a RNN require various tricks (e.g. padding, packing) that we will explore in this article. First, lets 
...
between `king` and `queen` is the gender. So logically, if we remove the vector of `male` to ̀`king` and add the vector
...
But, since we want to predict the most probable tokens, we will use the softmax function (see here if softmax does not ring a bell).
...
Now, let's set our training constants. We first have the CUDA device used for training if one is present. Secondly, 
we set the batch size (i.e. the number of elements to see before updating the model), the learning rate for the optimizer
...
> See here for more explanation why setting seed does not grand full reproductibility.
...
Now let's download our dataset; for simplicity, the data was already split into a train, valid and test. Also, the dataset
 is pickled for simplicity (a Python `list`). Here the code to download it.
...
If we take a look at the training dataset, it's a list of 80,000 tuples where the first element is the full address, 
and the second element is a list of the tag (the ground truth).
...
As explain, before (ref Word Embeddings) 
...
## Poutyne Experiment
> Disclaimer that I'm a dev on Poutyne so I will present code using this framework. See [here](https://poutyne.org/) the project.
...
### Results
Here my results after `10` epochs (results should be similar but not exacly the same), we can see that blah blah.
